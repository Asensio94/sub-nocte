"""Fase 2: meteorología histórica por radar (reanálisis ERA5 servido por Open-Meteo) y rasgos por noche.

Open-Meteo (https://open-meteo.com, CC BY 4.0, sin clave para uso no comercial) sirve ERA5 horario desde 1940
en la malla de 0,25° y ERA5-Land (9 km) para las variables de superficie. Para operar se cambiará al pronóstico
del mismo proveedor o a ECMWF Open Data; los nombres de variable son los mismos.

El viento a 100 m es el mejor proxy disponible sin clave del flujo en la capa de vuelo (200-1.500 m).
"""
...
# El archivo de Open-Meteo acepta variables en niveles de presión (925/850/700 hPa) pero las devuelve vacías
# para todo el periodo (comprobado 2018-2026): solo sirve superficie y 100 m. Para el modelo definitivo
# hay que añadir los niveles de presión desde el Climate Data Store de Copernicus (requiere clave personal).
HOURLY = [
...
def fetch_radar_meteo(radar: str, lat: float, lon: float, years: list[int], out_dir: Path, log=print) -> pd.DataFrame:
    """Descarga año a año (respuestas de ~9.000 horas × 18 variables) y guarda data/meteo/{radar}.parquet."""
...
def night_features(meteo: pd.DataFrame, nights: pd.DataFrame) -> pd.DataFrame:
